// =================================================================
// COMPLETE SCRIPT - BANKING & INSURANCE WITH DEPARTMENTS (v5 - REVISED)
// =================================================================
...
// =================================================================
// SECTION 3.8: LINK SUB-MODULES TO REQUIRED ROLES (REVISED)
// NOTE: This section ensures that every Sub-Module defined in the
// script is connected to at least one Role.
// =================================================================
...
// --- Credit Scoring Sub-Modules ---
MATCH (sm_cr_a:SubModule {name: "Data Foundation & Integration"})
MATCH (sm_cr_b:SubModule {name: "Feature Engineering Module"})
MATCH (sm_cr_c:SubModule {name: "Credit Scoring Model (Baseline)"})
MATCH (sm_cr_d:SubModule {name: "Fairness & Bias Mitigation Layer"})
MERGE (sm_cr_a)-[:REQUIRES_ROLE]->(devops)
MERGE (sm_cr_b)-[:REQUIRES_ROLE]->(ds)
MERGE (sm_cr_c)-[:REQUIRES_ROLE]->(ds)
MERGE (sm_cr_c)-[:REQUIRES_ROLE]->(mlOps)
MERGE (sm_cr_d)-[:REQUIRES_ROLE]->(ds)
MERGE (sm_cr_d)-[:REQUIRES_ROLE]->(ai);

// --- Fraud Detection Sub-Modules ---
MATCH (sm_fd_a:SubModule {name: "Streaming Data Ingestion Layer"})
MATCH (sm_fd_b:SubModule {name: "Rule-Based Engine Upgrade"})
MATCH (sm_fd_c:SubModule {name: "Anomaly Detection Model"})
MATCH (sm_fd_d:SubModule {name: "Alert Prioritization & Explainability"})
MERGE (sm_fd_a)-[:REQUIRES_ROLE]->(ai)
MERGE (sm_fd_a)-[:REQUIRES_ROLE]->(devops)
MERGE (sm_fd_b)-[:REQUIRES_ROLE]->(ai)
MERGE (sm_fd_c)-[:REQUIRES_ROLE]->(ds)
MERGE (sm_fd_c)-[:REQUIRES_ROLE]->(mlOps)
MERGE (sm_fd_d)-[:REQUIRES_ROLE]->(ds)
MERGE (sm_fd_d)-[:REQUIRES_ROLE]->(ai);

// --- Customer Service Sub-Modules ---
MATCH (sm_cs_a:SubModule {name: "Chatbot for FAQs"})
MATCH (sm_cs_b:SubModule {name: "Virtual Assistant for Transactions"})
MATCH (sm_cs_c:SubModule {name: "Sentiment Analysis & Escalation"})
MATCH (sm_cs_d:SubModule {name: "Agent Assist Tools"})
MERGE (sm_cs_a)-[:REQUIRES_ROLE]->(ai)
MERGE (sm_cs_b)-[:REQUIRES_ROLE]->(ai)
MERGE (sm_cs_b)-[:REQUIRES_ROLE]->(devops)
MERGE (sm_cs_c)-[:REQUIRES_ROLE]->(ds)
MERGE (sm_cs_c)-[:REQUIRES_ROLE]->(ai)
MERGE (sm_cs_d)-[:REQUIRES_ROLE]->(ai);

// --- Claims Processing Sub-Modules ---
MATCH (sm_cl_a:SubModule {name: "Claims Intake Portal"})
MATCH (sm_cl_b:SubModule {name: "OCR & Document Processing"})
MATCH (sm_cl_c:SubModule {name: "Claims Triage & Routing"})
MATCH (sm_cl_d:SubModule {name: "Assessment Models"})
MERGE (sm_cl_a)-[:REQUIRES_ROLE]->(devops)
MERGE (sm_cl_b)-[:REQUIRES_ROLE]->(ai)
MERGE (sm_cl_b)-[:REQUIRES_ROLE]->(mlOps)
MERGE (sm_cl_c)-[:REQUIRES_ROLE]->(ai)
MERGE (sm_cl_d)-[:REQUIRES_ROLE]->(ds)
MERGE (sm_cl_d)-[:REQUIRES_ROLE]->(mlOps);

// --- Next Best Action Sub-Modules ---
MATCH (sm_nba_a:SubModule {name: 'Customer 360 Data Layer'})
MATCH (sm_nba_b:SubModule {name: 'Propensity Modeling Engine'})
MATCH (sm_nba_c:SubModule {name: 'Recommendation Orchestrator'})
MATCH (sm_nba_d:SubModule {name: 'A/B Testing Framework'})
MERGE (sm_nba_a)-[:REQUIRES_ROLE]->(devops)
MERGE (sm_nba_b)-[:REQUIRES_ROLE]->(ds)
MERGE (sm_nba_b)-[:REQUIRES_ROLE]->(mlOps)
MERGE (sm_nba_c)-[:REQUIRES_ROLE]->(ai)
MERGE (sm_nba_d)-[:REQUIRES_ROLE]->(ds);

// --- HR Analytics Sub-Modules ---
MATCH (sm_hr_a:SubModule {name: 'Resume Parser & Matcher'})
MATCH (sm_hr_b:SubModule {name: 'Attrition Risk Scorer'})
MATCH (sm_hr_c:SubModule {name: 'Skills Taxonomy Engine'})
MATCH (sm_hr_d:SubModule {name: 'Performance Analytics'})
MERGE (sm_hr_a)-[:REQUIRES_ROLE]->(ai)
MERGE (sm_hr_b)-[:REQUIRES_ROLE]->(ds)
MERGE (sm_hr_c)-[:REQUIRES_ROLE]->(ds)
MERGE (sm_hr_c)-[:REQUIRES_ROLE]->(ai)
MERGE (sm_hr_d)-[:REQUIRES_ROLE]->(ds);

// --- Security Operations Sub-Modules ---
MATCH (sm_secops_a:SubModule {name: "Threat Intelligence Dashboard"})
MATCH (sm_secops_b:SubModule {name: "Automated Incident Response"})
MATCH (sm_secops_c:SubModule {name: "Behavioral Anomaly Detection"})
MATCH (sm_secops_d:SubModule {name: "False Positive Reduction Engine"})
MERGE (sm_secops_a)-[:REQUIRES_ROLE]->(devops)
MERGE (sm_secops_b)-[:REQUIRES_ROLE]->(ai)
MERGE (sm_secops_b)-[:REQUIRES_ROLE]->(devops)
MERGE (sm_secops_c)-[:REQUIRES_ROLE]->(ds)
MERGE (sm_secops_c)-[:REQUIRES_ROLE]->(ai)
MERGE (sm_secops_d)-[:REQUIRES_ROLE]->(ds);

// =================================================================
// SECTION 3.9: PAIN POINT AUDIT & CORRECTIONS (NEW)
// =================================================================

// --- Connect the 'High Customer Churn Rate' pain point ---
MATCH (pp_cust_churn:PainPoint {name: 'High Customer Churn Rate'})
MATCH (rb:Sector {name: 'Retail Banking'})
MATCH (ob:Sector {name: 'Online Banking'})
MATCH (li:Sector {name: 'Life Insurance'})
MATCH (hi:Sector {name: 'Health Insurance'})
MATCH (dept_mkt:Department {name: 'Marketing'})
MERGE (rb)-[:EXPERIENCES]->(pp_cust_churn)
MERGE (ob)-[:EXPERIENCES]->(pp_cust_churn)
MERGE (li)-[:EXPERIENCES]->(pp_cust_churn)
MERGE (hi)-[:EXPERIENCES]->(pp_cust_churn)
MERGE (dept_mkt)-[:EXPERIENCES]->(pp_cust_churn);

// --- Add more context for 'Generic Customer Communications' ---
MATCH (pp_generic_comm:PainPoint {name: 'Generic Customer Communications'})
MATCH (pb:Sector {name: 'Private Banking'})
MERGE (pb)-[:EXPERIENCES]->(pp_generic_comm);
